  ...
  <div class="container py-5">
    ...
    <div class="row">
      <div class="col-md-6">
        ...
        <!-- 在父元件將資料綁定到子元件，就可以在 子元件 NewestRestaurants 中使用 props  -->
        <!-- 這樣子元件就取得了一個叫 restaurants 的屬性，而其值為 "restaurants" -->
        <Newest-restaurants v-bind:restaurants="restaurants" />
      </div>
      <div class="col-md-6">
        <!-- 最新評論 NewestComments-->
        <h3>最新評論</h3>
      ...
      <!-- pascal & camel case are both accepted in vue -->
      <!-- 會自動辨認 將兩種寫法都視為相同元件 -->
    </div>
  </div>
</template>
...
<script>
// 引入 Navtabs 組件，這樣才能在 export component 之後
// 在這份 .vue 檔案的 template 中使用 <Navtabs /> 標籤
...
export default {
  ...
  components: {
    // 下面這些是從別的地方引入的 Vue 元件
    // 添加到 components 裡，代表可以在 template 中使用
    // 在 components 註冊過的元件 就可以在 template 使用
    Navtabs,
    NewestRestaurants,
    NewestComments,
  },
